...
public final class HTTPProtocol: NWProtocolFramerImplementation {
    ...
    public init(framer: NWProtocolFramer.Instance) {
        ...
        settings.on_message_complete = {
            $0?.framer.messageFinished()
            return 0
        }
        settings.on_headers_complete = {
            $0?.framer.headerFinished()
            return 0
        }
    ...
    private func emit(
        _ message: NWProtocolFramer.Message,
        to framer: NWProtocolFramer.Instance
    ) {
        let ok = framer.deliverInputNoCopy(
            length: 0,
            message: message,
            isComplete: true)
        assert(ok)
    ...
    private func emit(
        _ data: UnsafeBufferPointer<Int8>,
        to framer: NWProtocolFramer.Instance
    ) {
    ...
    private func headerFinished() {
        ...
        if isRequest {
            ...
            return emit(
                .init(
                    method: method, path: url, headers: headers,
    ...
    private func processDataForState(
        _ state: ParseState,
...
extension http_method {
    fileprivate var stringValue: String { return String(cString: http_method_str(self)) }
}
extension http_status {
    fileprivate var reason: String { return String(cString: http_status_str(self)) }
}
extension http_errno {
    fileprivate var name: String { return String(cString: http_errno_name(self)) }
...
extension UnsafeMutablePointer where Pointee == http_parser {
    fileprivate var framer: HTTPProtocol {
