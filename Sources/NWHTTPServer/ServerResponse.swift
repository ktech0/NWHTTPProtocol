...
open class ServerResponse {
     ...
     */
    open func writeHead(status: HTTPStatus? = nil, headers: [String: String] = [:]) {
        assert(!didWriteHead)
        ...
        let message = NWProtocolFramer.Message(
            ...
            headers: headerArray)
        let context = NWConnection.ContentContext(identifier: "HTTPResponseHead", metadata: [message])
    ...
    open func write(_ data: Data) -> Bool {
        ...
        let context = NWConnection.ContentContext(identifier: "HTTPResponseBody", metadata: [message])
    ...
    open func end() {
        ...
        let context = NWConnection.ContentContext(identifier: "HTTPResponseEnd", metadata: [message])
    ...
    public func flushIfPossible() {
        ...
        connection.send(content: content,completion: .contentProcessed(handleWriteResult(_:)))
    }
     ...
     *
     */
    open func sendJSON(_ value: Any, options: JSONSerialization.WritingOptions = []) throws {
        if contentType == nil { contentType = "application/json; charset=UTF-8" }
    ...
    /**
     * Send a file from the filesystem to the client.
     * Sets appropriate Content-Type and Content-Length headers.
     * Returns true if the file was successfully read and sent.
     */
